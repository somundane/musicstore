<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ConfirmPurchase.aspx.cs" Inherits="FinalProj.ConfirmPurchase" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title></title>
    <link href="Content/bootstrap.css" rel="stylesheet" />
    <link href="Content/bootstrap-theme.css" rel="stylesheet" />
    <link href="Styles/masterstyle.css" rel="stylesheet" />
    <script src="Scripts/jquery-1.9.1.js"></script>
    <script src="Scripts/bootstrap.js"></script>
    <link rel="stylesheet" type="text/css" href="Styles/preset.css" />

</head>

<body>
    <div class="panel panel-warning">
        <div class="panel-heading">
            <h2>Confirm Purchase</h2>
        </div>
        <form id="form2" runat="server">
            <div class="panel-body" >
                <h4><kbd>You are about to purchase these items:</kbd></h4>

                <div class="d-inline" style="font-size: 15px; margin-top: 15px; text-align: left;">
                    <asp:Label ID="lblCart" runat="server" Text=""></asp:Label>
                </div>
                <div class="d-inline" style="font-size: 15px; margin-top: 15px; text-align: left;">
                    <b><asp:Label ID="lblTotal" runat="server" Text=""></asp:Label></b>
                </div>
                <asp:Panel ID="pnlOut" runat="server"></asp:Panel>

                <div class="form-group">
                    <asp:Button ID="btnCancel" runat="server" Text="Cancel" CssClass="btn btn" OnClick="btnCancel_Click" />
                    <asp:Button ID="btnSubmit" runat="server" Text="Submit" CssClass="btn btn-success" OnClick="btnSubmit_Click" />
                </div>
            </div>
        </form>
    </div>
</body>
</html>
